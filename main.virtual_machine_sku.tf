...
locals {
  #narrow the sku list 
  no_restriction = var.resource_type == "vm" ? [for sku in data.azapi_resource_list.vm[0].output.value : sku.name
    if
  ...
  vm_find_duplicates = var.resource_type == "vm" ? { for sku in data.azapi_resource_list.vm[0].output.value : sku.name => sku... } : {} #group the output to ensure duplicates can be identified
  vm_duplicates      = var.resource_type == "vm" ? [for key, value in local.vm_find_duplicates : key if length(value) > 1]   : []           #any group with more than 1 value is a duplicate

  vm_map_conversion              = var.resource_type == "vm" ? { for sku in data.azapi_resource_list.vm[0].output.value : sku.name => sku if(!contains(local.vm_duplicates, sku.name) && contains(local.no_restriction, sku.name)) } : {} #convert the output to a map so we can work with the keys
  vm_map_capabilities_conversion = var.resource_type == "vm" ? { for sku in local.vm_map_conversion : sku.name => { for capability in sku.capabilities : capability.name => capability } } : {}                                          #convert the capabilities a map so we can work with the keys
  vm_map_location_conversion     = var.resource_type == "vm" ? { for sku in local.vm_map_conversion : sku.name => { for location in sku.locationInfo : location.location => location } } : {}                                           #convert the locationInfo to a map so we can work with the keys 
  vm_location_capabilities       = var.resource_type == "vm" ?{ for key, value in local.vm_map_location_conversion : key => try({ for capability in value[var.location].zoneDetails[0].capabilities : capability.name => capability }, {}) } : {}

  #Create separate lists of skus based on the conditions input for each capability 
  vm_per_element_valid_skus = var.resource_type == "vm" ? {
  ...
  vm_cache_map = {
    sku = try(tolist(local.vm_valid_skus)[random_integer.deploy_sku.result], "no_current_valid_skus_for_filter")
    sku_list = tolist(local.vm_valid_skus)
  }
